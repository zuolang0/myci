...
class Reserve extends CI_Controller{
	...
	public function index(){
		...
		#$data['province']=$this->area->get_city();
		#加载日历
		#设置验证规则
	...
	/*	#固定参数
		$param['op']='try';
		$param['aid']='10';
		$param['trail_channel_from']='Gome';*/
		#用户信息
		$name=$this->input->post('name');
		$mobile=$this->input->post('mobile');
		$param['name']=urlencode($name);
		$param['mobile']=urlencode($mobile);
		#车型
		$car_series=$this->input->post('car_series');
		if (!empty($car_series)) {
			$car_series_name=$this->car->get_name_by_groupId($car_series);
			$param['car_series']=urlencode($car_series);
			$param['car_series_name']=urlencode($car_series_name);
		}
		$province=$this->input->post('province');
		#省份相关参数处理
		if (!empty($province)) {
			$province_name=$this->area->get_name_by_regionId($province);
			$param['region_id_1']=urlencode($province);
			$param['region_id_1_name']=urlencode($province_name);
			$city=$this->input->post('city');
			if (!empty($city)) {
				$city_name=$this->area->get_name_by_regionId($city);
				$county=$this->input->post('county');
				$param['region_id_2']=urlencode($city);
				$param['region_id_2_name']=urlencode($city_name);
				if (!empty($county)) {
					$county_name=$this->area->get_name_by_regionId($county);
					$param['region_id_3']=urlencode($county);
					$param['region_id_3_name']=urlencode($county_name);
				}
			}
		}
		#经销商
		...
		if (!empty($dealer_id)) {
			...
			$param['dealer_id']=urlencode($dealer_id);
			$param['dealer_code_name']=urlencode($dealer_code_name);
			$param['dealer_code']=urlencode($dealer_code);
		}
		...
		if (strtotime($visit_time)) {
			$param['visit_time']=urlencode($visit_time);
		}
		if (strtotime($order_time)) {
			$param['order_time']=urlencode($order_time);
		}
		$url='http://www.changan.com.cn/api.php?op=try&aid=10';
		var_dump($param);
	...
	public function area_file(){
		...
		#省
		$province=$this->area->get_city_by_type('10541002');
		#市
		$city=$this->area->get_city_by_type('10541003');
		#区
		$town=$this->area->get_city_by_type('10541004');
